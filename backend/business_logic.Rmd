...
#' @param simulator full simulations results data frame
...
plot_interval <- function(simulator, params, from_to = c(0, 1)) {
  from <-  from_to[1]
  to <- from_to[2]
...
```{r}
plot_interval(res$simulator,params = res$params, from_to = c(1.5,2.9))
```
...
#     dat %>% mutate(`arrival type` = "all") ,
#     dat.j %>% mutate(`arrival type` = "joined"),
#     dat %>% filter(join == 0) %>% mutate(`arrival type` = "balked")
#   )
#
#   # patience boxplots
...
#     dat.ext %>%
#     ggplot() +
#     aes(x = sqrt(patience), fill = `arrival type`) +
...
#' @export
#'
pltSegments <- function(params,simulator_data, segment_n = 24) {
  range01 <- function(x) {
    (x - min(x)) / (max(x) - min(x))
  } # for future use
  ...
  simulator %>%
    ...
    group_by(segment) %>%
    summarize(
      segment_joined = mean(total_joined),
      segment_arrived = mean(total_arrived),
      segment_p = mean(total_joined / total_arrived)
    ) %>%
    ungroup() %>%
...
```{r}
res <- full_simulation(n = 1000,params = param_list(gamma = 100,lambda_0 = 50,theta = 1,eta = 1,mu = 1,s = 50),m = 1e4)
plot_patience(simulator = res$simulator,params = res$params)
```
