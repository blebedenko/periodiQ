...
full_simulation <- function(n, params, m = 1e6){
  ...
  T_tilde <- arrivalTimes(m = m,params = params) # arrival times of ALL customers
  ...
  while (effective_counter < n + 1) {
    ...
    customer <- customerExp(params = params)  # generate patience and job size:
    B <- customer$Jobsize
    Y <- customer$Patience
  ...
  simulator <-
    dplyr::bind_cols(
      ...
      queue = Qj,
      patience = Yj) %>%
    mutate(time_of_day = arrival - trunc(arrival),
